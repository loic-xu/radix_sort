...
-   Merge Sort : Un algorithme de tri par merge qui divise le tableau en deux parties, trie chacune d'elles, puis mergene les sous-tableaux triés. Il est stable et garanti d'avoir une complexité de O(n log n) dans tous les cas, ce qui le rend plus fiable que Quick Sort.
...
one.simu.time(n, func = "quick_sort")
one.simu.time(n, func = "heap_sort")
one.simu.time(n, func = "merge_sort_Rcpp")
one.simu.time(n, func = "quick_sort_Rcpp")
...
for(i in 1:nbSimus){time3[i] <- one.simu.time(n, func = "quick_sort")}
for(i in 1:nbSimus){time4[i] <- one.simu.time(n, func = "quick_sort_Rcpp")}
...
- **Quick Sort** : La version en Rcpp est environ **19 fois plus rapide** pour n = 10,000 et **17 fois plus rapide** pour n = 20,000, offrant également un avantage important de l'implémentation en Rcpp.
...
- **Merge Sort** est **3.9 fois plus lent** que **Quick Sort** pour n = 10,000 et **5.1 fois plus lent** pour n = 20,000.
- **Merge Sort** est aussi légèrement plus lent que **Heap Sort**, avec un écart de **0.1** pour n = 10,000 et de **0.078** pour n = 20,000.
- Enfin, **Quick Sort** est **beaucoup plus rapide** que **Heap Sort**, avec un écart de **0.025** pour n = 10,000 et de **0.015** pour n = 20,000.


## Simulations avec `microbenchmark`

Vous avez besoin des packages `microbenchmark` et `ggplot2` pour exécuter les simulations et afficher les résultats (sous forme de diagrammes en violon). Nous comparons `merge_sort_Rcpp` avec `quick_sort_Rcpp` et `heap_sort_Rcpp` pour des tailles de données `n = 1000` et `n = 10000`.
...
benchmark_sorting <- function(n, times = 70)
{
  microbenchmark(
    ...
    quick_sort = one.simu(n, func = "quick_sort_Rcpp"),
...
- **Comparaison entre les algorithmes C++** : En analysant les résultats des algorithmes naïfs en C++, nous avons constaté que **le tri par tas (heap sort)** est l'algorithme le moins performant parmi ceux testés, tandis que **le tri rapide (quick sort)** se révèle être le plus rapide, comme l'indiquent les distributions de temps d'exécution observées dans les graphiques (notamment les violons).

Nous pouvons donc conclure que, bien que les algorithmes en C++ surpassent largement ceux en R en termes de performance, **le tri rapide (quick sort)** reste l'algorithme le plus efficace parmi les options disponibles dans cette première phase.

Dans la suite de notre étude, nous ne conserverons que **le tri rapide** comme algorithme de comparaison, étant donné sa supériorité en termes de performance.
...
Dans cette partie, on redéfini 'one.sime.time' et 'one.simu' mais cette fois avec radix sort, et on garde quick sort car nous en aurons besoin dans la suite de l'étude.
...
## Comparaisons Radix Sort et Quick Sort
...
nbRep <- 10
vector_n_quick <- exp(seq(log(1000), log(50000), length.out = nbSimus))  # Taille de données sur une échelle logarithmique
vector_n_quick <- round(vector_n_quick)
res_Quick <- benchmark_sorting("quick_sort_Rcpp", vector_n_quick, nbRep)

model <- lm(log(res_Quick$mean_time) ~ log(res_Quick$n))  # Régression log-log
print(summary(model))  # Affichage des résultats du modèle
slope_r <- coef(model)[2]  # Récupération du coefficient directeur (exposant)
cat("Estimated exponent:", slope_r, "\n")  # Affichage de l'exposant estimé
...
res_Quick$algo <- "QuickSort"
res_Radix$algo <- "RadixSort"

# Combine les deux
df_all <- rbind(res_Quick, res_Radix)

# Calcul des pentes (exposants) par algorithme
model_quick <- lm(log(mean_time) ~ log(n), data = subset(df_all, algo == "QuickSort"))
model_radix <- lm(log(mean_time) ~ log(n), data = subset(df_all, algo == "RadixSort"))

slope_quick <- round(coef(model_quick)[2], 3)
slope_radix <- round(coef(model_radix)[2], 3)

# Affichage des pentes
cat("Pente QuickSort :", slope_quick, "\n")
